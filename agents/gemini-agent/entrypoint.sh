...
# Start Gemini via an Expect script that sends INSTRUCTIONS.md, then stays interactive
tmux send-keys -t agent:0 "/usr/local/bin/run-gemini.expect" C-m

echo "[agent] Gemini started in tmux session 'agent' (attach via cockpit terminal)"
