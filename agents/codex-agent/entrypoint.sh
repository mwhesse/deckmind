...
# Start Codex via an Expect script that sends INSTRUCTIONS.md, then stays interactive
tmux send-keys -t agent:0 "/usr/local/bin/run-codex.expect" C-m

echo "[agent] Codex started in tmux session 'agent' (attach via cockpit terminal)"
